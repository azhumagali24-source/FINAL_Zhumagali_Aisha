...
-- increase specialization length for more detailed instructor expertise
alter table instructors
alter column specialization type varchar(150);

-- add timestamp to track when a course is updated
alter table courses
...
-- require student names to contain at least 2 characters
alter table students
add constraint chk_student_name_length
check (length(full_name) >= 2);

-- remove unused category description column
...
-- mark enrollments as completed when progress reaches 100%
update enrollments 
set status = 'completed' 
where progress_percent = 100;

-- upgrade long-duration courses to Advanced level
update courses 
...
-- test deletion of refunded and failed payments
-- rollback is used to keep data unchanged after verification
begin;
